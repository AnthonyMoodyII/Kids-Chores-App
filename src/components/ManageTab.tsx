...
import { Users, Plus, Trash2, ListChecks, UserPlus, AlertCircle } from 'lucide-react';
...
export function ManageTab({
  ...
  const handleRemoveKid = async (id: string) => {
    if (
      !window.confirm(
        'Removing this child will also delete all their active chores. Continue?',
      )
    )
      return;
    try {
  ...
  const handleRemoveTemplate = async (templateId: string) => {
    ...
    if (activeLinked.length > 0) {
      const linkedKids = Array.from(
        new Set(
          activeLinked.map(
            c => kids.find(k => k.id === c.assignedTo)?.name || 'Unknown child',
          ),
        ),
      ).join(', ');
  ...
  const handleToggleMandatory = async (templateId: string) => {
    try {
      const response = await fetch(
        `${API_URL}/api/templates/${templateId}/toggle-mandatory`,
        { method: 'POST' },
      );
  ...
  const handleAssign = async (e: React.FormEvent) => {
    ...
    const toAssign = Array.from(selectedKidIds).filter(
      kidId =>
        !chores.some(
          c => c.templateId === selectedTemplateId && c.assignedTo === kidId && !c.isArchived,
        ),
    );
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <section className={`${cardSurface} p-6 md:p-8`}>
        ...
        {kids.length === 0 ? (
          ...
          <p className="py-10 text-center italic text-slate-400">
            Add at least one chore to the library.
          </p>
        ) : (
                ...
                {kids.map(k => {
                  ...
                  return (
                    <button
                      ...
                      className={`${btnBase} ${btnPress} rounded-full border-2 px-4 py-2 text-sm font-bold ${
                        on
