  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async tokenResponse => {
      ...
      try {
        ...
        const res = await fetch(`${API_URL}/api/parent/google-auth`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (result.success) {
          ...
          setError(result.error || 'Google account not authorized.');
        }
      } catch {
  ...
  return (
    <div className={`${cardSurface} mx-auto max-w-md p-8 md:p-10`}>
      ...
      <button
        ...
        className={`${btnBase} ${btnPress} inline-flex w-full items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white py-3.5 font-black text-slate-700 shadow-sm hover:border-slate-300 hover:bg-slate-50 disabled:pointer-events-none disabled:opacity-50`}
      >
        <GoogleIcon />
